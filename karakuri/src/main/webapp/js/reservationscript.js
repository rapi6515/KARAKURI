document.addEventListener('DOMContentLoaded', function() {
	...
	function updateScheduleTable(clickedDate) {
		currentlySelectedDate = clickedDate; // クリックされた日付を保持
		...
		console.log("該当日の予約:", todaysReservations);
        ...
        todaysReservations.forEach(res => {
            ...
            if (!reserveHourStr) { return; } // 時間がなければスキップ
            ...
            if (planIdStr === '101') { columnIndex = 1; } 
            else if (planIdStr === '102') { columnIndex = 2; } 
            else if (planIdStr === '103') { columnIndex = 3; }
            ...
            if (columnIndex !== -1) {
                ...
                if (targetCell) {
                    ...
                    // console.log(`印をつけました: ${scheduleTime} / プラン列 ${columnIndex}`); // デバッグ完了したら消してもOK
                } else {
                    console.warn(`時間割表のセルが見つかりません: ${targetCellSelector}`);
                }
            } else {
                 console.warn(`不明なプランIDです: ${planId}`);
            }
        });
    ...
    function handleScheduleClick(clickedCell) {
        ...
        if (clickedColumnIndex === 1) { clickedPlanId = '101'; } 
        else if (clickedColumnIndex === 2) { clickedPlanId = '102'; } 
        else if (clickedColumnIndex === 3) { clickedPlanId = '103'; }
