...
<body>
	...
	<% if (!"editComplete".equals(mode) && !"deleteComplete".equals(mode) && reservation != null) { %>
		...
		<% if ("edit".equals(mode)) { %>		
			...
			<form action="<%= request.getContextPath() %>/ReservationUpdateServlet" method="post">
		        <button type="submit">②予約内容変更確定</button>
		    </form>
	        
        <% } else if ("delete".equals(mode)) { %> 
	        <form action="<%= request.getContextPath() %>/ReservationDeleteServlet" method="post" onsubmit="return confirm('本当に予約を取消しますか？');">
	            <button type="submit">予約を取消する</button>
	        </form>
        <% } else { %>  
	        
	        <form action="<%= request.getContextPath() %>/CompleteReservationServlet" method="post">
    ...
    <% if ("deleteComplete".equals(mode)|| "editComplete".equals(mode)|| "edit".equals(mode)) { %>
	    <form action="<%= request.getContextPath() %>/jsp/KarakuriTop.jsp" method="psto">
